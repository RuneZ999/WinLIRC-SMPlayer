...
;    SMPlayer. See the examples below.

; This script requires AutoHotkey 1.0.38.04 or later.
; HISTORY OF CHANGES
; 2020:
; Fixed for working with Unicode version too (why871)
; Added controls for SMPlayer
; Added GUI at key pressed, make sure that SMPlayer has AlwaysOnTop disabled
; March 2, 2007:
...
Menu, Tray, Tip, SMPlayer WinLIRC Reciver
...
IfWinExist, SMPlayer
{
	...
	WinGetPos, X, Y, Width, Height, SMPlayer
	Send {space}
...
IfWinExist, SMPlayer
{
	...
	WinGetPos, X, Y, Width, Height, SMPlayer
	Send {4}
	guidisp("Brightness 4% Increase",X, Y)
}
else
{
	guidisp("SMPlayer NOT running",0, 0)
...
IfWinExist, SMPlayer
{
	...
	WinGetPos, X, Y, Width, Height, SMPlayer
	Send {3}
	guidisp("Brightness 4% Decrease",X, Y)
}
else
{
	guidisp("SMPlayer NOT running",0, 0)
